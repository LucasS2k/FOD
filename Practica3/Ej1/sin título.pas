...
procedure bajaEmpleado(var Es: empleados);
var
busqueda: integer;
nuevo: empleados;
reg: empleado;
begin
	writeln('ingrese el codigo de empleado a eliminar');
	readln(busqueda);
	reset(Es);
	rewrite(nuevo);
	leer(Es, reg);
	while(reg.num <> busqueda) do begin
		write(nuevo, reg);
		read(Es,reg);
	end;
	read(Es, reg);
	...
	close(Es);
	close(nuevo);
	assign(Es, 'respaldo.dat');
	assign(nuevo, 'maestro.dat');
end;
...
BEGIN
	...
	writln('8 para eliminar a un empleado');
	readln(opcion);
	case opcion of
	1: buscarEmpleado(ES);
